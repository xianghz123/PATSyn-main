...
from pctsyn.domain import build_public_domain, save_public_domain
from pctsyn.utils import load_config, save_json


def parse_args():
    parser = argparse.ArgumentParser(description="Build the public domain for PCTSyn.")
...
def main():
    ...
    print(f"[PCTSyn] Building public domain: {config['dataset']['name']}")
    ...
    print(f"[PCTSyn] Cells: {domain_stats['num_cells']}")
    print(f"[PCTSyn] Legal transitions: {domain_stats['num_edges']}")
    print(f"[PCTSyn] |Omega_q|: {domain_stats['omega_q_size']}")
    print(f"[PCTSyn] Saved: {domain_path}")
