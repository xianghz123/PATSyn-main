...
from pctsyn.preprocessing import prepare_dataset, save_processed_dataset
from pctsyn.utils import load_config, save_json, set_seed


def parse_args():
    parser = argparse.ArgumentParser(description="Prepare trajectories for PCTSyn.")
...
def main():
    ...
    print(f"[PCTSyn] Preparing dataset: {config['dataset']['name']}")
    ...
    print(f"[PCTSyn] Admitted trajectories: {len(trajectories):,}")
    print(f"[PCTSyn] Saved: {trajectory_path}")
