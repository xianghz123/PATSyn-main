...
from patsyn.recovery import recover_model, save_model
from patsyn.reporting import generate_private_reports, save_reports
from patsyn.synthesis import (
    generate_synthetic_trajectories,
    save_sampled_conditions,
    save_synthetic_dataset,
)
from patsyn.utils import load_config, peak_rss_mib, save_json, set_seed


def parse_args():
    parser = argparse.ArgumentParser(description="Run the PATSyn synthesis pipeline.")
...
def main():
    ...
    print(f"[PATSyn] Dataset: {config['dataset']['name']}")
    print(f"[PATSyn] epsilon: {epsilon}")
    print(f"[PATSyn] Seed: {seed}")
    print(f"[PATSyn] Input trajectories: {num_input:,}")
    ...
    print(f"[PATSyn] Reporting: {reporting_time:.2f} s")
    print(f"[PATSyn] Recovery: {recovery_time:.2f} s")
    print(f"[PATSyn] Synthesis: {synthesis_time:.2f} s")
    print(f"[PATSyn] Method total: {method_time:.2f} s")
