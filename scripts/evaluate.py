...
from pctsyn.metrics import evaluate_all
from pctsyn.preprocessing import load_processed_dataset
from pctsyn.synthesis import load_sampled_conditions, load_synthetic_dataset
from pctsyn.utils import load_config, save_json, set_seed


def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate PCTSyn outputs.")
...
def main():
    ...
    print(f"[PCTSyn] Saved: {metrics_json_path}")
    print(f"[PCTSyn] Saved: {metrics_csv_path}")
